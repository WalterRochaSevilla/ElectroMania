...
export class CartService {
    ...
    readonly totals = computed<CartTotals>(() => {
        ...
        return { total_items, subtotal, impuestos, total };
    });
    readonly isEmpty = computed(() => this.itemsSignal().length === 0);
    readonly itemCount = computed(() => this.totals().total_items);
    ...
    async updateQuantity(id: number, cantidad: number): Promise<void> {
        ...
        this.itemsSignal.update(items => items.map(item => item.id === id ? { ...item, cantidad } : item));
        if (!this.authService.isAuthenticated()) {
            this.saveToStorage();
        }
    }
    async increaseQuantity(id: number, maxStock?: number): Promise<void> {
        ...
        this.itemsSignal.update(items => items.map(i => i.id === id ? { ...i, cantidad: newQuantity } : i));
        if (this.authService.isAuthenticated()) {
            try {
                await this.postUpdateQuantity(id, 1);
            }
            catch (error) {
                console.error('Failed to increase quantity', error);
                this.itemsSignal.update(items => items.map(i => i.id === id ? { ...i, cantidad: item.cantidad } : i));
            }
        }
        else {
            this.saveToStorage();
